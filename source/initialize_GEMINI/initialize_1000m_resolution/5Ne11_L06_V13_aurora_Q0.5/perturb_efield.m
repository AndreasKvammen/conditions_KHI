...
params = struct(...
  'v0', -650.0, ...       % background flow value, actually this will be turned into a shear in the Efield input file
  'densfact', 3.0, ...    % factor by which the density increases over the shear region - see Keskinen, et al (1988)
  'ell', 6.0e3, ...  % scale length for shear transition
  'B1val', -47500e-9, ...
...
create_particles(cfg, xg, params)
...
% !!!! NOTE SCALE FACTOR !!!
scalefact=10;  % This causes ne_mean = 4.955e11 
% !!!! NOTE SCALE FACTOR !!!
for isp=1:lsp-1
...
function create_particles(p, xg, params)
...
xgmlon=squeeze(xg.phi(1,:,1)*180/pi);
% xgmlat=squeeze(90-xg.theta(1,1,:)*180/pi);
x2i=interp1(xgmlon,xg.x2(3:xg.lx(2)+2),precip.mlon,'linear','extrap');
% x3i=interp1(xgmlat,xg.x3(3:lx3+2),E.mlat,'linear','extrap');

for i = i_on:i_off
    for ilat=1:precip.llat
        precip.Qit(:,ilat,i) = 1e-5 + p.Qprecip*(1/2 + 1/2 * tanh(x2i/params.ell));
    end %for
